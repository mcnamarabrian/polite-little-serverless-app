...
- polite-greeting - Code for the application's Lambda function.
- events - Invocation events that you can use to invoke the function.
- tests - Unit tests for the application code. 
- template.yaml - A template that defines the application's AWS resources.

The application uses Lambda functions. It is defined in the `template.yaml` file in this project. You can update the template to add AWS resources through the same deployment process that updates your application code.
...
The SAM CLI installs dependencies defined in `polite-greeting/requirements.txt`, creates a deployment package, and saves it in the `.aws-sam/build` folder.
...
```bash
polite-little-serverless-app$ sam local invoke PoliteGreetingFunction --event events/event.json
```
...
```bash
polite-little-serverless-app$ sam logs -n PoliteGreetingFunction --stack-name polite-little-serverless-app --tail
```
